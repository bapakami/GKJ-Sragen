...
class Kelola_Pembangunan extends CI_Controller {
	...
	{
        $data['data']=$this->M_Gereja_Persembahan->DataList($this->session->userdata('gereja_id'),'Pembangunan');
		$data['NamaPepantan'] = $this->M_Gereja_Persembahan->GetNamaPepantan($this->session->userdata('gereja_id'));
		$this->load->view('template/header');
        $this->load->view('template/menu');
		$this->load->view('userGereja/Pembangunan', $data);
		$this->load->view('template/footer');
	}

	public function insertData()
    {
        $typeName = 'Pembangunan';
        $data = array (
        ...
        redirect('AdminGereja/Kelola_Pembangunan');
    }
