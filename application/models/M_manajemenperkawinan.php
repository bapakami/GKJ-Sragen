...
class M_manajemenperkawinan extends CI_Model
{
	...
	function getStatistik($gereja,$parts)
	{
		 ...
		 COUNT(IF(a.status_perkawinan='Belum Menikah',1, NULL)) as BelumMenikah,
		 COUNT(IF(a.status_perkawinan='Duda',1, NULL)) as Duda,
		 COUNT(IF(a.status_perkawinan='Janda',1, NULL)) as Janda, 
		 COUNT(IF(a.status_perkawinan='Menikah',1, NULL)) as Menikah,
		 COUNT(IF(a.status_perkawinan='Single Parent',1, NULL)) as SingleParent 
		 from jemaats a 
		 Left join gereja b ON a.gerejaid = b.id 
		 WHERE a.gerejaid=$gereja 
		 and a.status = 'Hidup' 
		 and a.status_perkawinan in ($parts)");
